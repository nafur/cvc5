...
std::vector<poly::Variable> sortML(const Constraints::ConstraintVector& polys, std::unique_ptr<VOMLState>& state) {
  if (!state) {
    state.reset(new VOMLState("vo-ml.model"));
  }
  std::size_t selection = (*state)(cad::NRAFeatures(polys));
  switch (selection) {
    case 0: return sortBrown(polys);
    case 1: return sortByid(polys);
    case 2: return sortTriangular(polys);
    default:
      Notice() << "Learned heuristic selected unsupported variable ordering: " << selection;
      return sortBrown(polys);
  }
}
#else
class VOMLState {};
...
#ifdef CVC4_USE_DLIB
    case VariableOrderingStrategy::ML: return sortML(polys, state_ml);
#endif
