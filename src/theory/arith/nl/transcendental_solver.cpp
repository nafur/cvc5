                                        ...
                                        const std::vector<Node>& xts)
{
      ...
      if (consider)
      {
        ...
        if (aa != a)
        {
          ...
          if (mvaa != mvaaa)
          {
            ...
            d_im.addLemma(cong_lemma, Inference::CONGRUENCE);
          }
        }
        else
        {
  ...
  for (const Node& a : trNeedsMaster)
  {
    ...
    Trace("nl-ext-lemma") << "NonlinearExtension::Lemma : purify : " << lem
                          << std::endl;
    ArithLemma nlem(lem, LemmaProperty::PREPROCESS, nullptr, Inference::T_PURIFY_ARG);
    d_im.addLemma(nlem);
  }
...
void TranscendentalSolver::processSideEffect(const ArithLemma& se)
{
...
void TranscendentalSolver::getCurrentPiBounds()
{
  ...
  d_im.addLemma(pi_lem, Inference::T_PI_BOUND);
}
  ...
  for (std::pair<const Kind, std::vector<Node> >& tfl : d_funcMap)
  {
    ...
    for (const Node& t : tfl.second)
    {
      ...
      if (d_tf_initial_refine.find(t) == d_tf_initial_refine.end())
      {
        ...
        if (!lem.isNull())
        {
          d_im.addLemma(lem, Inference::T_INIT_REFINE);
        }
      }
    }
  }
        ...
        for (unsigned i = 0, size = sorted_tf_args[k].size(); i < size; i++)
        {
          ...
          if (!tval.isNull())
          {
            ...
            if (!mono_lem.isNull())
            {
              ...
              d_im.addLemma(mono_lem, Inference::T_MONOTONICITY);
            }
          }
                                                   ...
                                                   unsigned d)
{
  ...
  if (is_tangent)
  {
    ...
    // Figure 3 : line 9
    d_im.addLemma(lem, Inference::T_TANGENT);
  }
  else if (is_secant)
  {
    ...
    ArithLemma nlem(lem, LemmaProperty::NONE, nullptr, Inference::T_SECANT);
